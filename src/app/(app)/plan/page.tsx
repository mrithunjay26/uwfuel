    ...
    <div className={`rounded-[16px] border p-3 ${isActive ? "border-success/40 bg-success/5" : "border-line bg-surface-2"}`}>
      <div className="flex items-start gap-2">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-[11px] text-ink-soft">
            {plan.daily_totals.calories.toLocaleString("en-US")} kcal · {plan.daily_totals.protein}g · {formatMoney(plan.daily_totals.cost)} · {plan.meals.length} meal{plan.meals.length === 1 ? "" : "s"}
          </p>
        </div>
      ...
      <div className="mt-2.5">
        ...
        {clashDays.length > 0 && (
          <p className="mt-1.5 flex items-start gap-1 text-[10.5px] font-semibold text-warning">
            ...
            A meal lands during a class on {clashDays.map((d) => d.label).join(", ")} — move that meal's time or the class.
          </p>
        )}
      </div>
    </div>
