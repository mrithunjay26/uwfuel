...
export function MealDetailSheet({ item, onClose, onLog, isLogging }: MealDetailSheetProps) {
  return (
    <BottomSheet
      ...
      label={item?.name}
      snapPoints={[0.6, 0.92]}
      footer={item ? <DetailFooter item={item} onLog={onLog} isLogging={isLogging} /> : null}
    >
      {item ? <DetailBody item={item} onClose={onClose} /> : null}
    </BottomSheet>
  );
}

function DetailBody({ item, onClose }: { item: FlatMenuItem; onClose: () => void }) {
...
function DetailFooter({ item, onLog, isLogging }: { item: FlatMenuItem; onLog?: (item: FlatMenuItem) => void; isLogging?: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {orderable && needsSearchAssist(item) && (
