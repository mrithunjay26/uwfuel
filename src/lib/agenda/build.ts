...
import { isMealRecommendationCandidate, scoreMealRecommendation } from "@/lib/menu/flattenMenu";
...
export function buildDailyAgenda(input: {
  ...
  const mealBudget = input.budget.combinedTodayGuide > 0 ? Math.max(input.budget.combinedTodayGuide, 8) : 18;
  const meal = input.menu
    .filter((item) => isMealRecommendationCandidate(item) && item.price <= mealBudget)
    .sort((a, b) => scoreMealRecommendation(b, mealBudget) - scoreMealRecommendation(a, mealBudget))[0];
  if (meal) items.push({ id: "meal-now", kind: "meal", priority: nextClass ? 85 : 105, eyebrow: "Complete meal nearby", title: meal.name, detail: `${meal.location_name} · ${Math.round(meal.calories)} cal · ${Math.round(meal.protein_grams)}g protein · $${meal.price.toFixed(2)}`, href: `/menu?location=${encodeURIComponent(meal.location_id)}&item=${encodeURIComponent(meal.unique_key)}`, action: "View details" });
